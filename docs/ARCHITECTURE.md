...
scripts/gen_frontend.py` at the top is written by that script** — edit the
generator, not the copy, and `tests/test_frontend_guards.py` fails if the two
disagree.
...
* **A new fieldtype** → `scripts/field_types.py`, which is checked against
  Frappe's own list; the SPA follows from the generator.
* **A new doctype** → `scripts/gen_doctypes.py`. It must also get a customer or
  operator surface, or `tests/test_no_desk.py` fails — there is no desk.
