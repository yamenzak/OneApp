...
| `lib/shell/` | What the chrome knows: `session` and `user`, `nav` and `sidebar`, `breakpoint`, `theme` and `appearance`, `icons`, `shortcuts`, `notifications`, `settings` (the dialog's open state) and `mail` (the column's folders). |
